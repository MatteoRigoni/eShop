...
namespace eShop.DataStore.SQL.Dapper
{
    public class OrderRepository : IOrderRepository
    {
        ...
        public IEnumerable<OrderItem> GetLineItemsByOrderId(int orderId)
        {
            var sql = "SELECT * FROM OrderLineItem WHERE OrderId = @OrderId";
            var lineItems = dataAccess.Query<OrderItem, dynamic>(sql, new { OrderId = orderId });
        ...
        {
            var sql = "SELECT * FROM [ORDER] WHERE DateProcessing is null";
            return dataAccess.Query<Order, dynamic>(sql, new { });
        }

        public IEnumerable<Order> GetProcessedOrders()
        {
            var sql = "SELECT * FROM [ORDER] WHERE DateProcessing is not null";
            return dataAccess.Query<Order, dynamic>(sql, new { });
        }
        ...
        public void UpdateOrder(Order order)
        {
            ...
            dataAccess.ExecuteCommand<List<OrderItem>>(sql, order.LineItems);
        }
    }
}
